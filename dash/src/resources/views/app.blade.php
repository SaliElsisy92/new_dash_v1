...
<head>
    ...
    <!-- CSS Files -->
    <!-- Bootstrap css -->
		<link href="{{ url('dashboard/assets/dashtemplate/plugins/bootstrap/css/bootstrap.css') }}" rel="stylesheet" id="style"/>
    ...
    @if (!empty($fields))
        ...
        <script src="https://cdn.ckeditor.com/ckeditor5/35.1.0/super-build/ckeditor.js" defer></script>
...
<body class="app sidebar-mini {{ session('DARK_MODE',config('dash.DARK_MODE')) == 'on'?'dark-mode':'' }}
 {{ app()->getLocale() == 'ar' ? 'rtl' : '' }} {{ session('sidenav_toggled','close') == 'open'?'sidenav-toggled':'' }}">
	...
	<!-- Back to top -->
		...
		<!--Moment js-->
		<script src="{{ url('dashboard/assets/dashtemplate') }}/plugins/moment/moment.js"></script>
