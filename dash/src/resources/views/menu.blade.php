    ...
    <aside class="app-sidebar">
        <div class="app-sidebar__logo">
            <a class="header-brand" href="{{ dash('/') }}">
                <img src="{{ $DASHBOARD_ICON }}" class="header-brand-img desktop-lgo" style="max-height:41px"
                    alt="{{ env('APP_NAME') }}">
                <img src="{{ $DASHBOARD_ICON }}" class="header-brand-img dark-logo" style="max-height:41px"
                    alt="{{ env('APP_NAME') }}">
                <img src="{{ $DASHBOARD_ICON }}" class="header-brand-img mobile-logo" style="max-height:41px"
                    alt="{{ env('APP_NAME') }}">
                <img src="{{ $DASHBOARD_ICON }}" class="header-brand-img darkmobile-logo" style="max-height:41px"
                    alt="{{ env('APP_NAME') }}">
            </a>
        </div>
            ...
            <div class="main-menu">
                <div class="app-sidebar__user d-none">
                    <div class="dropdown user-pro-body text-center">
                        <div class="user-pic">
                            <img src="{{ admin()?->photo ?? url('dashboard/assets/img/admin.png') }}" alt="user-img"
                                class="avatar-xxl rounded-circle mb-1">
                        </div>
                        <div class="user-info">
                            <h5 class=" mb-2">{{ admin()?->name }}</h5>
                            <span
                                class="text-muted app-sidebar__user-name text-sm">{{ admin()?->admingroup?->name }}</span>
                        </div>
                    </div>
                </div>
                    ...
                    @if (isset($loadInNavigationPagesMenu['top']) && !empty($loadInNavigationPagesMenu['top']))
                        ...
                        @foreach ($loadInNavigationPagesMenu['top'] as $page)
                            @if ($page['displayInMenu'])
                                <li class="slide">
                                    <a href="{{ url($DASHBOARD_PATH . '/page/' . $page['RouteName']) }}"
                            ...
                            <li class="slide {{ $dash_active_collapse ? 'is-expanded' : '' }}">
                                <a class="side-menu__item {{ $dash_active_collapse ? 'active' : '' }}"
                                    ...
                                    <span class="side-menu__label">
                                        ...
                                        class="angle fa {{ app()->getLocale() != 'en' ? 'fa-angle-left' : 'fa-angle-right' }}"></i>
                                </a>
                                ...
                                <ul class="slide-menu {{ $dash_active_collapse ? 'open' : '' }}">
                                    ...
                                    @foreach ($groups as $group)
                                        @if (isset($group['displayInMenu']) && $group['displayInMenu'])
                                            <li
                                                class="sub-slide {{ request()->segment(3) == $group['resourceName'] ? 'is-expanded' : '' }}">
                                                <a href="{{ url($DASHBOARD_PATH . '/resource/' . $group['resourceName']) }}"
        ...
        {{ request()->segment(3) == $group['resourceName'] ? 'active is-expanded' : '' }}">
                                        ...
                                        @elseif(!isset($group['displayInMenu']) && empty($group['displayInMenu']))
                                            ...
                                            <li class="sub-slide {{ $dash_active_collapse_sub ? 'is-expanded' : '' }}">
                                                <a class="sub-side-menu__item {{ $dash_active_collapse_sub ? 'active is-expanded' : '' }}"
                                                    ...
                                                    <i
                                                        class="sub-angle fa {{ app()->getLocale() != 'en' ? 'fa-angle-left' : 'fa-angle-right' }}"></i>
                                                </a>
                                                ...
                                                <ul class="sub-slide-menu">
                                                    @foreach ($group as $subgroups)
                                                        @if (isset($subgroups['displayInMenu']) && $subgroups['displayInMenu'])
                                                            <li>
                                                                <a href="{{ url($DASHBOARD_PATH . '/resource/' . $subgroups['resourceName']) }}"
                                                                    class="sub-slide-item {{ request()->segment(3) == $subgroups['resourceName'] ? 'active' : '' }}">
                                                                    <i class="sidemenu_icon">
                                                                        {!! $subgroups['icon'] ?? '' !!}
                                                                    </i>
