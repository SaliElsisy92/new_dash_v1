<script type="text/javascript">
    $(document).ready(function(){
        var modal_edit = `
        <div class="modal fade inline_modal" id="boxModal_edit" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
           <div class="modal-dialog modal-lg modal-xl">
             <div class="modal-content">
               <div class="modal-header">
                   <h5 class="modal-title edit_model_label" id="boxModal_editLabel"> </h5>
                 <i data-bs-dismiss="modal"  style="cursor: pointer;" class="fa fa-times"></i>
               </div>
               <div class="modal-body inline_modal_content content_boxModal_edit"></div>
           </div>
         </div>
       </div>
        `;
     ...
     $(document).on('click','.edit_relation_row',function(){
       ...
       $.ajax({
        ...
        cache: false,
        beforeSend:function(){
            $('.content_boxModal_edit').empty();

            $('.content_boxModal_edit').html(`
            <center>
        ...
        success:function(data){
            $('.content_boxModal_edit').html(data);
        }
       });
      return false;
     });
