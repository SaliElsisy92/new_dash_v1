        ...
        render: function(data,type,full,meta){
          ...
          if(imageCol?.trim() != null && Object.keys(imageCol?.trim()).length > 0){
            ...
            <img class="img-fluid rounded-circle img-thumbnail"
             ...
             data-bs-target="#avatar_image{{ $resource['resourceName']??'' }}`+data.id+`">
            `;
          }else{
            return '-';
          }
...
//Relationship statement End
    ...
    render:function (data, type, full, meta){
      ...
      var buttons = `
      @if($pagesRules['edit'])
      <a class="edit_relation_row" res_icon="{{ $resource['navigation']['icon'] }}" res_label="{{ $resource['navigation']['customName'] }}" res_name="{{ $resource['resourceName'] }}" record_id="`+data.id+`" class='m-1' href='javascript:void' ><i class='fa-regular fa-pen-to-square'></i></a>
      @endif
      @if($pagesRules['show'])
      <a class="show_relation_row" res_icon="{{ $resource['navigation']['icon'] }}" res_label="{{ $resource['navigation']['customName'] }}" res_name="{{ $resource['resourceName'] }}" record_id="`+data.id+`" class='m-1' href='javascript:void' ><i class='fa-solid fa-eye'></i></a>
      @endif
      @if($pagesRules['destroy'])
      <a href="#" action='{{ url($DASHBOARD_PATH.'/resource/'.$resource['resourceName']) }}/`+data.id+`' rowid="`+data.id+`" class="deleteRow{{ $resourceName }}" class='m-1'><i class='fa-solid fa-trash-can'></i></a>
      @endif
      `;
