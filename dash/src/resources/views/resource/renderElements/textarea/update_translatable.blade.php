...
@endphp
	<li class="nav-item" role="tabinput">
		<button class="nav-link p-2 {{ $transIndex==0?'active':'' }}" id="{{ $inputID.$random }}-tab" data-bs-toggle="tab" data-bs-target="#{{ $inputID.$random }}" type="button" role="tab" aria-controls="{{ $inputID.$random }}" aria-selected="true">{{ $value }}</button>
	</li>
...
<textarea name="{{ $key }}[{{ $field['attribute'] }}]"
		...
		id="{{ $inputName }}">{{ method_exists($model,'translate')? $model->translate($key)->{$field['attribute']}:$model->{$field['attribute']} }}</textarea>
